...
Date: 07/29/2022
Homework Problem #3
Description of Problem: Analyze the SSE for both deceased and surviving patient records
"""
import math
import pandas as pd
import numpy as np
...
def get_loss_func(x_train, x_test):
    ...
    # print(len(x_train[x]), len(x_train[y]))
...
if __name__ == "__main__":
    ...
    heartfailure_clinical_records.columns = ["age", "anaemia", "CPK_level", "diabetes", "ejection_fraction", "high_blood_pressure", "platelets", "serum_creatinine", "serum_sodium", "sex", "smoking", "time", "death"]
    print(len(heartfailure_clinical_records))
    ...
    print(len(heartfailure_clinical_df))
    ...
    df_survived= heartfailure_clinical_df.loc[(heartfailure_clinical_df['death']=='0'), ["serum_creatinine","serum_sodium"]]
    df_deceased= heartfailure_clinical_df.loc[(heartfailure_clinical_df['death']=='1'), ["serum_creatinine","serum_sodium"]]
    print(len(df_survived))
    print(len(df_deceased))
    # Split the sruvived patient records 50/50
    x_train_0, x_test_0= train_test_split(df_survived.iloc[:,:2], test_size = 0.5, random_state= 3)
    print(len(df_survived))
    # print(len(x_train_0))
    get_loss_func(x_train_0, x_test_0)

    x_train_1, x_test_1= train_test_split(df_deceased.iloc[:,:2], test_size = 0.5, random_state= 3)

    get_loss_func(x_train_1, x_test_1)
    

    """
    Problem 3.1
    """
    # the GLM_1 (y = alogx + b) has the best SSE for deceased patients while GLM_2 (logy = alogx + b) has the best SSE for survived patients
    """
    Problem 3.2
    """
    # the GLM_2 (logy = alogx + b) has the worst SSE for deceased patients while cubic (y = ax^3 + bx^2 + cx + d) has the worst SSE for survived patients
